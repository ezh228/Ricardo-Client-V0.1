...
public class WorldClient extends World
{
    ...
    {
        if (Client.moduleManager.getModule("CupBoardRender").isEnabled()){
            if (entityIn instanceof EntityArmorStand && !(mc.player.getDistanceToEntity(entityIn) > 200) && getBlockState(new BlockPos(entityIn.posX, entityIn.posY, entityIn.posZ)).getBlock() != Blocks.AIR){
                return;
            }
        }
    ...
    {
        if (Client.moduleManager.getModule("CupBoardRender").isEnabled()) {
            if (entityToSpawn instanceof EntityArmorStand){
                for (Entity entityArmorStand : loadedEntityList){
                    if (entityArmorStand instanceof EntityArmorStand){
                        if (entityArmorStand.getDistanceToEntity(entityToSpawn) < 0.5f){

                            super.removeEntity(entityArmorStand);
                            this.entityList.remove(entityArmorStand);
                        }
                    }
                }
            }
